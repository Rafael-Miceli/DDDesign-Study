...
namespace AppointmentScheduling.Data.Repositories
{
    public class ScheduleRepository : IScheduleRepository, IDisposable
    {
        ...
        public Schedule GetScheduleForDate(int companyId, DateTime date)
        {
            Guid scheduleId = GetScheduleIdForCompany(companyId);
            ...
            return new Schedule(scheduleId, DateTimeRange.CreateOneDayRange(date), companyId, appointments);
        }
        ...
        private Guid GetScheduleIdForCompany(int companyId)
        {
            // gets one schedule entity, and then pulls the id from that tracked EF entity
            // Guid scheduleId = _context.Schedules.FirstOrDefault(s => s.CompanyId == companyId).Id; 
